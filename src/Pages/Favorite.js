...
constructor(props) {
  ...
  handleRemoveFavorite =  async(id) => {
      const {uid} = this.context.auth.user;
      await this.firebaseRef
      .ref('Pengguna/Pelanggan/' + uid +'/Favorite/'+id).remove()
  }
  ...
  render(){
    ...
    // for (const [key, value] of Object.entries(this.state.favorite)) {
    //   console.log(`${key}: ${value.nama}`);
    // }
    favoriteKey.map((data)=> {
      ...
      {favoriteKey.length > 0 ?(
        ...
        <TouchableOpacity>
            <ListItem bottomDivider>
                ...
                <ListItem.Content>
                <ListItem.Title style={{fontSize:14}}>{this.state.favorite[key].nama}</ListItem.Title>
                <ListItem.Subtitle>{this.state.favorite[key].alamat}</ListItem.Subtitle>
                </ListItem.Content>
            </ListItem>
        </TouchableOpacity>
        </Swipeable>
